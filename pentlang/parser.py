"""
Parser for PentLang programming language.

Parses tokens into an Abstract Syntax Tree (AST).
Supports balanced quinary and five-state logic.
"""
...
class BooleanLiteral(ASTNode):
    """Boolean literal node (AbsoluteYes, MaybeYes, Unknown, MaybeNo, AbsoluteNo)."""
    def __init__(self, value: Any):  # 2, 1, 0, -1, or -2
        self.value = value
...
class Parser:
    ...
    def parse_primary(self) -> ASTNode:
        ...
        if token.type in [TokenType.ABSOLUTE_YES, TokenType.MAYBE_YES, TokenType.UNKNOWN, 
                          TokenType.MAYBE_NO, TokenType.ABSOLUTE_NO]:
            self.advance()
            if token.type == TokenType.ABSOLUTE_YES:
                value = 2
            elif token.type == TokenType.MAYBE_YES:
                value = 1
            elif token.type == TokenType.UNKNOWN:
                value = 0
            elif token.type == TokenType.MAYBE_NO:
                value = -1
            else:  # ABSOLUTE_NO
                value = -2
